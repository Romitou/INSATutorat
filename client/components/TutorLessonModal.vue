...
const props = defineProps<{
  show: boolean
  initialData?: TutoringLesson
}>()

const emit = defineEmits<{
  (e: 'close'): void
  (e: 'submit', lesson: TutoringLesson): void
}>()
...
const errors = ref<{ date?: string; time?: string; duration?: string; content?: string }>({})
...
const isValid = computed(() => {
  ...
  if (!content.value.trim()) {
    errors.value.content = t("contentRequired")
  }
  ...
  <div v-if="show" class="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
    <div class="bg-white rounded-lg w-full max-w-md shadow-xl p-6 relative">
      <h2 class="text-lg font-semibold text-zinc-800 mb-4">
        {{ props.initialData?.id ? t("edit") : t("newFeminine") }} {{ $t("lesson") }}
      </h2>

      <div class="space-y-4">

        <div>
          <label class="block text-sm font-medium text-zinc-700">Date</label>
          <input
              type="date"
              v-model="date"
        ...
        <div>
          <label class="block text-sm font-medium text-zinc-700">{{  $t("content") }}</label>
          <textarea rows="4" v-model="content" class="mt-1 w-full border rounded-md p-2 text-sm resize-none"></textarea>
          <p v-if="errors.content" class="text-sm text-red-500 mt-1">{{ errors.content }}</p>
        </div>
      </div>
