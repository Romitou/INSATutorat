...
interface SubjectWithMaxTutees extends Subject {
  maxTutees: number;
}
...
const subjects = ref<SubjectWithMaxTutees[]>([])
const selectedSubjects = ref<SubjectWithMaxTutees[]>([])
...
onMounted(async () => {
  ...
  if (subjectsResult.ok) {
    subjects.value = await subjectsResult.json() as SubjectWithMaxTutees[];
  }

  const registrationsResult = await useApiFetch(`/campaign/${campaignId}/tutor/registrations`, {
    method: 'GET',
    headers: {'Content-Type': 'application/json'}
  })
...
async function submitChoices() {
  try {
    const response = await useApiFetch(`/campaign/${campaignId}/tutor/registrations`, {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({
        subjects: selectedSubjects.value.map(subject => subject.id),
        maxTutees: selectedSubjects.value.map(subject => subject.maxTutees),
      })
    });
          ...
          <div>
            ...
            >
              <template #item="{ element, index }">
                <div class="p-3 mb-2 rounded-lg bg-red-100 text-red-800 font-medium shadow-inner flex flex-col gap-1">
                  <div class="flex items-center justify-between">
                    <span>
                      <span class="text-sm text-zinc-500 mr-2">{{ index + 1 }}.</span>
                      {{ element.shortName }} - {{ element.name }}
                    </span>
                  </div>
                  <div class="flex items-center gap-2 mt-1 text-sm">
                    <label class="text-zinc-700" for="maxTutees">{{ $t('maxTuteesLabel') }}</label>
                    <input
                        v-model.number="element.maxTutees"
                        class="w-16 rounded border border-zinc-300 px-2 py-1 text-zinc-800 text-sm focus:outline-none focus:ring-1 focus:ring-red-500"
                        max="10"
                        min="2"
                        type="number"
                    />
