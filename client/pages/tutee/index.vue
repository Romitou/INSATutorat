...
import type {Campaign, TuteeRegistration} from "~/types/api";

interface CampaignTuteeOverview extends Campaign {
  assignments: TuteeRegistration[]
  availability: Slot[];
}
...
const campaigns = ref<CampaignTuteeOverview[]>([])
...
onMounted(async () => {
  const assignmentsResult = await useApiFetch('/assignments/tutee', {
    method: 'GET',
    headers: {'Content-Type': 'application/json'}
  })

  if (assignmentsResult.ok) {
    campaigns.value = (await assignmentsResult.json()) as CampaignTuteeOverview[];
  }
...
-              <h3 class="text-md font-medium text-gray-800">{{ $t("myTutorsTitle") }}</h3>
+              <h3 class="text-md font-medium text-gray-800">{{ t("myTutorsTitle") }}</h3>
             </div>
...
-                  👤 <span class="font-semibold">{{ $t("tutorTitle") }} :</span> {{ assignment.tutor.lastName }}
-                  {{ assignment.tutor.firstName }}
+                  👤 <span class="font-semibold">{{ t("tutorTitle") }} :</span> {{ assignment.tutor?.lastName || '' }} {{ assignment.tutor?.firstName || '' }}
                 </p>
                 <NuxtLink
